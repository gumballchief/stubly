...
const ALL_MAINNET = [...ROLES, "client_mainnet", "deployer_mainnet", "treasury_mainnet"];
...
async function main() {
  ...
  const roles = [...ROLES, ...(args.includes("--with-client") ? ["client_mainnet"] : []), ...(args.includes("--with-deployer") ? ["deployer_mainnet"] : []), ...(args.includes("--with-treasury") ? ["treasury_mainnet"] : [])];
